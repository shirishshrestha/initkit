# InitKit CLI - Examples & Use Cases ## Table of Contents
...
- [CI/CD Integration](#cicd-integration) ## Quick Start Examples ### 1. Interactive Mode (Recommended for First-Time Users) ```bash
initkit
...
``` Skips the project name question, but shows all other prompts. ## Frontend Projects ### React App with TypeScript and Tailwind ```bash
...
- Styling: Plain CSS ## Backend Projects ### Express API with PostgreSQL ```bash
...
- Features: ESLint, Prettier, dotenv ## Full Stack Projects ### MERN Stack Application ```bash
...
- Language: TypeScript
- Styling: Tailwind CSS
- Features: All development tools ## Library Projects ### NPM Package ```bash
...
- Project Type: Library
- Language: JavaScript
- Features: ESLint, Prettier, Jest ## Advanced Usage ### Custom Package Manager ```bash
# Use yarn
...
- Testing
- CI/CD environments
- Scripts ## CI/CD Integration ### GitHub Actions Workflow Create `.github/workflows/init-project.yml`: ```yaml
...
- GitHub Actions
- EditorConfig **Result:** Publishing-ready npm package ## Pro Tips ### 1. Template Quick Creation Save time with template flag:
```bash
